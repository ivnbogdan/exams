...
- [x] **H3** Vercel: confirm Hobby plan under Settings → Billing. Import the repo as a project, framework Next.js. First deploy may fail; that is fine. *(before Stage 6)*
- [x] **H4** Vercel project → Storage → Create Database → **Neon**, Free plan, connect to all environments. Copy `DATABASE_URL` to the agent. *(Stage 1)*
...
- [x] **H7** Cloudflare R2: enable R2 (needs a payment method, free tier is 0 €). Bucket `exams-ro-files`. Bucket Settings → Custom Domains → `files.exams.ro`. Enable the r2.dev public dev URL as a temporary base URL. Manage R2 API Tokens → create token, **Object Read & Write**, this bucket only. Give the agent: account id, access key id, secret access key, bucket name, public base URL. *(Stage 2)*
- [x] **H8** On the machine: `vercel login`, then in the repo `vercel link`. *(Stage 1, optional but useful)*
- [x] **H9** Paste the values from H4 and H7 to the agent for `.env.local`, and add the same in Vercel → Settings → Environment Variables for Production and Preview. *(Stage 2 locally, Stage 6 on Vercel)*
- [ ] **H10** (after H5 and H6) Vercel → Settings → Domains → add `exams.ro` and `www.exams.ro`. Add the records Vercel shows into Cloudflare DNS with the proxy **off** (grey cloud). Remove the old A record. *(cutover)*
